...
function removeUsernameAlert(){
    ...
    if(format.test(username)){
        alert.classList = "alerts"
        alert.innerHTML=
            "<i class=\"fa-solid fa-triangle-exclamation\"></i> Username contém caratacteres invalidos"
        return false
    }
    if(username.length>30){
        alert.classList = "alerts"
        alert.innerHTML=
            "<i class=\"fa-solid fa-triangle-exclamation\"></i> Username demasiado grande"
        return false
    }
    else{
        alert.innerHTML=""
...
function submitFormToDeleteUser(form){
    let xhttp = new XMLHttpRequest()
    xhttp.onload = function (){
        let res = xhttp.responseText
        let name = form["user"].value
        if(res == 'ok'){
            let div = document.getElementById("deleteResults")
            console.log(div)
            div.classList.toggle('sucess')
            div.innerHTML=" " +
                "<i class=\"fa-solid fa-trash\"></i> " +
                "Utilizador "+name+"  Eliminado"
            setTimeout(() => {  document.getElementById("deleteResults").classList.toggle('sucess');
                document.getElementById("deleteResults").innerHTML="" }, 5000);
            setTimeout(()=>{
